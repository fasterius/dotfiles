...
vim.schedule(function()
    ...
    -- Run `make` when fzf-native is installed or updated
    vim.api.nvim_create_autocmd("PackChanged", {
        callback = function(ev)
            local data = ev.data
            if
                data.spec.name == "telescope-fzf-native.nvim"
    ...
    vim.keymap.set(
        ...
        "<leader>fg",
        builtin.git_files,
        { desc = "[F]ind files inside [G]it repository" }
    )
    vim.keymap.set(
        "n",
        "<leader>ff",
    ...
    vim.keymap.set(
        ...
        "<leader>sg",
        _live_grep,
        { desc = "[S]earch inside [G]it repository " }
    )
    vim.keymap.set(
        "n",
        "<leader>sw",
        _grep_string,
        { desc = "[S]earch current [W]ord inside Git repository" }
    )
    vim.keymap.set(
        "n",
        "<leader>sh",
